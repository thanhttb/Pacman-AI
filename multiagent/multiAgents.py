...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minValue(gameState, agentNum, depth):
          if(terminalTest(gameState, depth)):
            return self.evaluationFunction(gameState)
          v = float('inf')
          legalActions = gameState.getLegalActions(agentNum)
          for a in legalActions:
            newState = gameState.generateSuccessor(agentNum, a)
            if agentNum < gameState.getNumAgents()-1:
              v = min(v, minValue(newState, agentNum+1, depth))
            else:
        ...
        def maxValue(gameState, depth):
          ...
          for a in gameState.getLegalActions(0):
            ...
            v = max(v, minValue(newState, 1, depth))
          return v
        ...
        maxAction = Directions.STOP # temp
        for a in newActions:
          resultState = gameState.generateSuccessor(0, a)
          ghostIndex = 1
          minVal = minValue(resultState, ghostIndex, self.depth)
          if(minVal > miniMaxValue):
            miniMaxValue = minVal
            maxAction = a
        return maxAction
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minValue(gameState, agentNum, depth, alpha, beta):
          if(terminalTest(gameState, depth)):
            return self.evaluationFunction(gameState)
          v = float('inf')
          legalActions = gameState.getLegalActions(agentNum)
          for a in legalActions:
            newState = gameState.generateSuccessor(agentNum, a)
            if agentNum < gameState.getNumAgents()-1:
              v = min(v, minValue(newState, agentNum+1, depth, alpha, beta))
            else:
        ...
        def maxValue(gameState, depth, alpha, beta):
          ...
          for a in gameState.getLegalActions(0):
            newState = gameState.generateSuccessor(0, a)
            v = max(v, minValue(newState, 1, depth, alpha, beta))
            if v > beta:
              return v
            alpha = max(alpha, v)
          return v
        ...
        posInf = float('inf')
        maxAction = Directions.STOP # temp
        v = maxValue(gameState, self.depth, negInf, posInf)
        for a in newActions:
          resultState = gameState.generateSuccessor(0, a)
          ghostIndex = 1
          minVal = minValue(resultState, ghostIndex, self.depth, negInf, posInf)
          if v == minVal:
            return a
        return v
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        for a in newActions:
          ...
          dict[item] = minVal
          item += 1
        median = len(minVals)/2
        medVal = indexToVal[median]
        finalAction = valToAction[medVal]
        return finalAction
